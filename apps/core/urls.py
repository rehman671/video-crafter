...
urlpatterns = [
    ...
    path(
        "register-after-payment/", register_after_payment, name="register_after_payment"
    ),
    ...
    path(
        "download-video/<int:video_id>/",
        proxy_video_download,
        name="proxy_video_download",
    ),
    # HAVE TO CHANGE
    path("password_reset/", index, name="password_reset"),
    path(
        "manage_subscription/", manage_subscription, name="manage_subscription"
    ),  # Add this URL pattern
    #    path("asset_library", index, name="asset_library"),
    #    path("recent_videos", index, name="recent_videos"),
    path(
        "billing_portal/", stripe_billing_portal, name="billing_portal"
    ),  # Add this URL pattern
    path(
        "upgrade-plan/<int:plan_id>/", upgrade_plan, name="upgrade_plan"
    ),  # Add plan upgrade URL
    path(
        "cancel-subscription/", cancel_subscription, name="cancel_subscription"
    ),  # Add subscription cancellation URL
    ...
    path(
        "delete-asset/<int:asset_id>/", delete_asset, name="delete_asset"
    ),  # Add asset deletion URL
    path(
        "rename-asset/<int:asset_id>/", rename_asset, name="rename_asset"
    ),  # Add asset rename URL
    ...
    path(
        "loading/<int:video_id>/", loading_view, name="loading"
    ),  # Add loading URL pattern
    ...
    path(
        "update-clip-transition/", update_clip_transition, name="update_clip_transition"
    ),
    path(
        "update-clip-sound-effect/",
        update_clip_sound_effect,
        name="update_clip_sound_effect",
    ),
    path(
        "api/sound-effect/<int:sound_effect_id>/",
        serve_sound_effect,
        name="serve_sound_effect",
    ),
]

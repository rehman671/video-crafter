...
class VideoProcessorService:
    ...
    def apply_background_music(self, bg_music: BackgroundMusic):
        ...
        try:
            ...
            if has_audio:
                # Create a silent version of the original audio for the part where background music will be
                if start_time > 0:
                    # Create a version of the original audio with silence during the background music part
                    silent_part_path = tempfile.mktemp(suffix='.mp3')
                    silence_cmd = [
                        "ffmpeg",
                        "-y",
                        "-i", original_audio_path,
                        "-af", f"volume=1:enable='between(t,0,{start_time})+between(t,{end_time},{video_duration})',volume=0:enable='between(t,{start_time},{end_time})'",
                        "-c:a", "mp3",
                        silent_part_path
                    ]
                    
                    print(f"Creating audio with silent section: {' '.join(silence_cmd)}")
                    try:
                        subprocess.run(silence_cmd, capture_output=True, check=True)
                    except subprocess.CalledProcessError as e:
                        print(f"Error creating silent section: {str(e)}")
                        # Just use the original audio
                        silent_part_path = original_audio_path
                else:
                    # If background music starts at the beginning, just use the original
                    silent_part_path = original_audio_path
                ...
                if music_positioned:
                    ...
                    # Mix the original audio (with silence during bg music) and the positioned background music
                ...
                for temp_file in [original_audio_path, silent_part_path, music_timing_path, 

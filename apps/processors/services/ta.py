...
class TextAudioAligner:
    ...
    def align_text_to_audio(self, audio_path: str, text_path: str, output_path: str, 
                           detect_speech_start: bool = True) -> str:
        """
        ...
        Args:
            ...
            output_path: Path where to save the alignment result
            detect_speech_start: If True, will detect and use actual speech start time,
                               rather than starting from the end of the previous word
        """
        try:
            ...
            # Common settings for speech start detection
            if detect_speech_start:
                # Enhanced settings to better detect when speech actually starts
                head_detection = (
                    "is_audio_file_detect_head=true|"
                    # More aggressive head detection to find actual speech start
                    "is_audio_file_detect_head_max=1.200|"  # Look up to 1.2 seconds into the audio
                    "is_audio_file_detect_head_min=0.020|"  # Detect even small pauses of 20ms
                )
            else:
                # Standard settings
                head_detection = (
                    "is_audio_file_detect_head=false|"  # Don't auto-detect speech start
                )
            ...
            if duration and duration > 30.0:
                ...
                config_string = (
                    ...
                    "is_audio_file_detect_tail_min=0.150|"
                    # Add the head detection settings
                    f"{head_detection}"
                    # Break text into smaller chunks for better alignment
                    "task_max_text_chunk_length=12.000|"
                    # Additional optimizations
                    "task_max_audio_length=0|"
                    "task_adjust_boundary_no_zero=true|"
                    # Include audio head/tail info in output
                    "os_task_file_head_tail_format=true|"
            ...
            # Remove any doubled pipe characters that might have been created

...
async function handleFolderUpload() {
    ...
    document.getElementById('uploadStatus').style.display = 'block';
    document.getElementById('uploadStatus').textContent = 'Analyzing folders...';
    ...
    if (Object.keys(subfolders).length === 0) {
        console.log("No subfolders found, zipping entire folder");
        document.getElementById('uploadStatus').textContent = 'No subfolders found, processing entire folder...';
    ...
    try {
        ...
        for (const [subfolderName, files] of Object.entries(subfolders)) {
            try {
                ...
                document.getElementById('uploadStatus').textContent = 
                    `Processing subfolder: ${subfolderName} (${folderIndex}/${totalSubfolders})`;
...
async function createZipFromSubfolder(files, subfolderName) {
    ...
    document.getElementById('uploadStatus').textContent = progressText;
    ...
    try {
        ...
        for (const file of files) {
            ...
            processedCount++;
            document.getElementById('uploadStatus').textContent = 
                `${progressText} - Adding file ${processedCount} of ${totalFiles}`;
        }
        
        // Generate the ZIP file
        document.getElementById('uploadStatus').textContent = `${progressText} - Compressing files...`;
        const content = await zip.generateAsync({
            type: "blob",
            compression: "DEFLATE",
            compressionOptions: { level: 6 }
...
function handleFolderFileChange(event) {
    ...
    // Count subfolders for better UI feedback
    if (folderFiles && folderFiles.length > 0) {
        ...
        if (subfolders.size > 0) {
            ...
            subfoldersText.textContent = `Contains ${subfolders.size} subfolder${subfolders.size > 1 ? 's' : ''}`;
